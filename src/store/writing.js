    ...
    promptTemplates: [
      {
        ...
        templates: {
          thesis:
            "During the time period between _____ and _____ in __(geographic location)__  there were significant changes in  __(AP Theme)__   such as  __(category)__,  __(category)__ , and __(category)__.",
          evidence: [
            [
  ...
  getters: {
    ...
    findFeedback: (state) => (i) => {
      ...
      if (!feedback.length) {
        ...
        feedback[0].content.contextualization.good =
          "The response situates the emergence of German leader Adolf Hitler in the aftermath of World War I.";
        feedback[0].content.contextualization.toImprove =
          "You could have added more contextualization to be more detailed ";
        feedback[0].content.thesis.good =
          "The response situates the emergence of German leader Adolf Hitler in the aftermath of World War I.";
        feedback[0].content.thesis.toImprove =
          "You could have added more contextualization to be more detailed ";
        feedback[0].content.conclusion.good =
          "The response situates the emergence of German leader Adolf Hitler in the aftermath of World War I.";
  ...
  mutations: {
    ...
    uploadUserInput(state, payload) {
      ...
      if (index > -1) {
        ...
        state.feedback[index].content.contextualization.userInput =
          payload.contextualization ||
          state.feedback[index].content.contextualization.userInput;
        state.feedback[index].content.thesis.userInput =
          payload.thesis || state.feedback[index].content.thesis.userInput;
        state.feedback[index].content.conclusion.userInput =
          payload.conclusion ||
          state.feedback[index].content.conclusion.userInput;

        if (payload.moduleVersion === "Expert") {
          state.feedback[index].labelled =
            payload.labelled || state.feedback[index].labelled;
          state.feedback[index].content.raw =
            payload.raw || state.feedback[index].content.raw;
        }
        ...
        for (let i in payload.evidence) {
          if (i > state.feedback[index].content.evidence.length - 1) {
            ...
            state.feedback[index].content.evidence[i].evidence.userInput =
              payload.evidence[i][0] || null;
            state.feedback[index].content.evidence[i].analysis.userInput =
              payload.evidence[i][1] || null;
          }
        }
      } else {
        const userInput = {
          ...
          labelled: payload.moduleVersion !== "Expert",
          score: null,
          content: {
            contextualization: {
