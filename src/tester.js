...
d3.timeline = function () {
  ...
  var mouseover = async function (d) {
    ...
    // var doc =  await db.collection("tester").doc('rzMOembAuSuFfBKeXb2i')

    // var array = await doc.get().then(function (doc) {
    // 			return doc.data().events;
    // 	})

    // array.forEach(doc => {
    // 	if(d.label == doc.label){
    // 		console.log(doc.label)
    // 		var x = parseInt(doc.label) + 1;
    // 		doc.label = x.toString()
    // 	}
    // })
    // console.log(array)
    // doc.set({
