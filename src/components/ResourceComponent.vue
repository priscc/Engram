<template>
  <div id="ResourceComp">
    ...
    <b-row class="pt-4">
      ...
      <b-col v-if="visibleVideos.length >= 2" class="text-right">
        ...
        <div class="view_all" v-b-modal="'modal-1'">
          View all ({{ visibleVideos.length }})<b-icon-chevron-right aria-hidden="true" />
        </div>
    ...
    <!-- TOP 3 (unchanged) -->
    <b-col v-for="(v, i) in visibleVideos.slice(0, 2)" :key="v._key" class="pt-1">
      <div class="video-box">
        <div class="ratio">
    ...
    <b-modal id="modal-1" size="xl" title="All videos" hide-footer @shown="onModalShown">
      <b-row>
        <b-col v-for="(v, i) in visibleVideos" :key="'modal-' + v._key" lg="6" md="6" sm="12" class="p-3">
