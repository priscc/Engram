...
export default {
	...
	methods: {
		...
		pullData () {
			db.collection('events').where('topicID', '==', this.$props.topicObj.topicID).get().then(function(querySnapshot) {
					if (querySnapshot.docs.length == 0) querySnapshot = undefined
					querySnapshot.forEach(function(doc) {
						var entry = doc.data()
						entry.id = doc.id
						entry.date = entry.date.slice(0, 4)
						entry.start = entry.date.slice(0, 4)
						entry.end = entry.date.slice(0, 4)
						entry.color = '#FF0000'
		...
		//////////////
			...
			DrawCircles (g) {
				...
				// g.selectAll('svg').data(Circledata).enter()
				// 	.append('text')
				// 	.attr('class', function (d) { return ('text' + d.title) })
		...
		// ChangeEvent: function(n){
		// 	console.log("in here", this.prvCircle)
		...
		// 	if( this.prvCircle != 0){
		// 		console.log("in here")
		// 		document.getElementById('circle' + this.prvCircle).style.stroke = 'none'
		// 	}
