  ...
  <div class="my-auto" style="height: min-content;">
    <div class="text-center my-auto" style="position: relative;">
      <svg
        ...
        width="145"
        height="145"
        viewBox="-75 -80 155 150"
        fill="none"
      >
        <path
          d="M0 -69 A69 69 0 1 1 0 69 69 69 0 1 1 0 -69"
          :stroke="currentTime >= maxTime - 10 ? '#8C30F5' : '#D9DBE1'"
          stroke-width="2"
        />
        <path
          :d="
            `M0 -69 A69 69 0 ${
              currentTime > maxTime / 2 ? 1 : 0
            } 1 ${endX} ${endY}`
          "
          stroke="#8C30F5"
          stroke-width="2"
        ...
        <circle
          ...
          stroke-width="1"
          fill="#D9DBE1"
          :class="{ 'passed-circle': landmark.value.passed }"
          :cx="landmark.value.cx"
          :cy="landmark.value.cy"
          :r="landmark.value.passed ? 8.5 : 6.5"
        ></circle>
      </svg>
      <div class="time">
      ...
      <span
        ...
        :class="{ 'passed-indicator': contextualization.passed }"
        >Contextualization ({{ 3 * multiplier }} min)</span
      >
      <span class="thesis" :class="{ 'passed-indicator': thesis.passed }"
        >Thesis ({{ 5 * multiplier }} min)</span
      >
      <span class="evidence1" :class="{ 'passed-indicator': evidence1.passed }"
        >Evidence #1 / Analysis ({{ 10 * multiplier }} min)</span
      >
      <span class="evidence2" :class="{ 'passed-indicator': evidence2.passed }"
        >Evidence #2 / Analysis ({{ 10 * multiplier }} min)</span
      >
      <span
        class="conclusion"
        :class="{ 'passed-indicator': conclusion.passed }"
        >Conclusion ({{ 5 * multiplier }} min)</span
      >
      <span class="revisions" :class="{ 'passed-indicator': revision.passed }"
        >Revisions ({{ 2 * multiplier }} min)</span
      >
    </div>
    <div class="mt-auto my-auto">
      <b-form-group class="form-border-top">
        ...
        <b-row align-h="center" class="g-5">
          <b-col cols="auto">
            <b-form-checkbox
              :disabled="checkboxable"
              v-model="fiftyPercentMultiplier"
            >
              I have 1.5x time {{
            }}</b-form-checkbox>
          </b-col>
          <b-col cols="auto">
            <b-form-checkbox
              :disabled="checkboxable"
              v-model="hundredPercentMultiplier"
            >
              I have 2x time</b-form-checkbox
            >
          </b-col>
        </b-row>
        <!-- </b-form-checkbox-group> -->
...
export default {
  setup() {
    //Checkbox
    const checkboxable = computed(
      () => fiftyPercentMultiplier.value || hundredPercentMultiplier.value
    );
    const fiftyPercentMultiplier = ref(false);
    const hundredPercentMultiplier = ref(false);
    //
    const multiplier = computed(() => {
      return fiftyPercentMultiplier.value
        ? 1.5
        : hundredPercentMultiplier.value
        ? 2
        : 1;
    });
    ...
    const frac = computed(
      () => (currentTime.value / maxTime.value) * Math.PI * 2
    );
    ...
    const contextualization = computed(() => {
      return {
        ...
        cx:
          Math.cos(
            ((10 * 60 * multiplier.value) / maxTime.value) * Math.PI * 2 +
              Math.PI * 0.5
          ) * -69,
        cy:
          Math.sin(
    ...
    const thesis = computed(() => {
      return {
        ...
        cx:
          Math.cos(
            ((13 * 60 * multiplier.value) / maxTime.value) * Math.PI * 2 +
              Math.PI * 0.5
          ) * -69,
        cy:
          Math.sin(
    ...
    const evidence1 = computed(() => {
      return {
        ...
        cx:
          Math.cos(
            ((18 * 60 * multiplier.value) / maxTime.value) * Math.PI * 2 +
              Math.PI * 0.5
          ) * -69,
        cy:
          Math.sin(
    ...
    const evidence2 = computed(() => {
      return {
        ...
        cx:
          Math.cos(
            ((28 * 60 * multiplier.value) / maxTime.value) * Math.PI * 2 +
              Math.PI * 0.5
          ) * -69,
        cy:
          Math.sin(
    ...
    const conclusion = computed(() => {
      return {
        ...
        cx:
          Math.cos(
            ((38 * 60 * multiplier.value) / maxTime.value) * Math.PI * 2 +
              Math.PI * 0.5
          ) * -69,
        cy:
          Math.sin(
    ...
    const revision = computed(() => {
      return {
        ...
        cx:
          Math.cos(
            ((43 * 60 * multiplier.value) / maxTime.value) * Math.PI * 2 +
              Math.PI * 0.5
          ) * -69,
        cy:
          Math.sin(
    ...
    return {
      ...
      currentTimeHoursMinutes,
      checkboxable,
      fiftyPercentMultiplier,
      hundredPercentMultiplier
    };
  }
};
</script>
