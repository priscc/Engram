<template>
    <b-form class="essay-area p-0" @submit.prevent="handleSubmit">
        <b-container fluid class="essay-area p-0">
            <b-row class="form-row mx-auto pt-5" align-h="center">
                <b-col>
                    <b-form-group
                        ...
                        :key="stream.title"
                        class="stream-droplet px-4"
                    >
                        <div
                            v-if="stream.title === 'Conclusion'"
                            class="add-evidence-button mx-auto user-responsivity"
                            @click="addEvidence"
                        >
                            ...
                            <img
                                ...
                                alt="Icon"
                                :src="
                                    require('@/assets/writing_feature/plus.svg')
                                "
                            />
                        </div>
                        <span class="area-title">{{ stream.title }}</span
                        ><br />
                        ...
                        <textareavue
                            ...
                            @fulfillRequest="
                                (data) => handleRequest(data, stream.template)
                            "
                        ></textareavue>
                        <div
                            v-if="stream.subTemplateSubtitle"
                            class="subsubtitle"
                        >
                            <div class="area-subtitle">
                                {{ stream.subTemplateSubtitle.subtitle }}
                            </div>
                            <textareavue
                                :moduleVersion="moduleVersion"
                                :dataRequested="dataRequested"
                                :string="stream.subTemplate.template"
                                :placeholder="stream.placeholder"
                                :bgColor="'#F5E07B'"
                                @click.native="
                                    setCurrentSection(
                                        stream.subTemplateSubtitle.title,
                                    )
                                "
                                @fulfillRequest="
                                    (data) =>
                                        handleRequest(data, stream.subTemplate)
                                "
                            ></textareavue>
                        </div>
                    </b-form-group>
            ...
            <b-row
                align-h="end"
                class="px-5 pb-5 my-4 mt-5 mt-sm-0 mt-md-0 mt-lg-0"
            >
                <b-button
                    id="purple"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const stream = ref([
            {
                ...
                subtitle:
                    "add 1 sentence of historical context relevant to the prompt",
            ...
            },
            {
                title: "Thesis",
                subtitle:
                    "fill in the Change & Continuity thesis statement template",
                template: thesis,
                palette: "#85CDF9",
            },
            {
        ...
        const totalEmits = computed(
            () => stream.value.length + evidences.value,
        );
