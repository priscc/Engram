  ...
  <b-container fluid>
    <b-row class="breakdown-container" align-h="center">
      ...
      <b-col class="my-0 p-0" cols="auto">
        <div
          class="accordion fitted-section"
          role="tablist"
          style="position: relative;"
          :class="{ expansive: currentSection }"
        >
          <div
            no-body
            class="mb-3 breakdown-colors border-0"
            v-for="(section, index) in sections"
            :key="section.name"
            :style="CssVars(section)"
            :class="{ 'is-active': currentSection === section.name }"
            @click="handleClick(section.name)"
          >
            <div
              header-tag="header"
              class="p-1 ps-4 pe-4 section-header border-0"
              role="tab"
              v-b-toggle="'accordion-' + index"
            >
              <!-- <b-button block variant="info">Accordion 1</b-button> -->
              {{ section.title }}
              <span class="when-open ms-auto breakdown_icon">
                <img
                  :src="require('@/assets/writing_feature/expand_more.svg')"
                  alt="Icon"
                />
              </span>
              <span class="when-closed ms-auto breakdown_icon">
                <img
                  :src="require('@/assets/writing_feature/expand_less.svg')"
                  alt="Icon"
                />
              </span>
            </div>
            <b-collapse
              :id="'accordion-' + index"
              :visible="currentSection === section.name"
              accordion="accrdion"
              role="tabpanel"
              class="border-0"
              style="opacity: 1;"
