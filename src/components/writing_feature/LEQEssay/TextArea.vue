  ...
  <div class="fontist" ref="container" @click="handleFocus">
    ...
    >
      <span
        class=""
        v-show="
          moduleVersion === 'Beginner' ||
            (moduleVersion === 'Intermediate' &&
              !advancedWriting[0].content.length &&
        ...
        <span v-for="(word, index) in finalString" :key="index">
          <span v-if="word.editable" class="" style="position: relative;">
            <span @focus="handleFocus" v-if="word.noTemplate" class="">
              <span
                class="no-template"
                :placeholder="placeholder ? placeholder : '....add text here'"
                :contenteditable="
                  moduleVersion === 'Intermediate' ? false : true
                "
                ...
                :style="{
                  padding: `0 ${
                    (word.placeholder.length - word.content.length) * 10 > width
                      ? width * 0.3
                      : (word.placeholder.length - word.content.length) * 4.1
                  }px`,
      ...
      <span
        v-show="
          moduleVersion === 'Intermediate' &&
            (!displayTemplatePlaceholder || advancedWriting[0].content.length)
        "
      >
        <span
          @blur="handleFocusout"
          ...
          @input="handleInput($event, advancedWriting[0])"
          >{{ advancedWriting[0].shallow_copy }}</span
        >
      </span>
    </div>
  </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    if (Object.prototype.toString.call(props.string) === "[object Array]") {
      ...
      if (props.moduleVersion === "Intermediate") {
...
.editable-textbox:focus-within {
  color: black;
  border: 1px solid var(--bg-color);
  border-right: 5px solid var(--bg-color);
  margin-right: -5px;
}
.editable-textbox:focus .text-blank,
.editable-textbox:focus-within .text-blank {
