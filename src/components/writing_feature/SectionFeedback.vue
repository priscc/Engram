<template>
  <b-container fluid class="white-contain-feedback">
    <div v-for="section in stream" :key="section.title" class="stacked-padding">
      <b-row
        align-h="between"
        class="header-control mb-4"
        style="position: relative;"
      >
        <h1 class="title-variant-feedback">{{ section.title }}</h1>
        <div
          class="point-given align-middle"
          :style="{ background: section.palette }"
        >
          <span class="title-variant-feedback align-middle">{{
            section.points
          }}</span>
        </div>
      </b-row>
      ...
      <b-row class="g-5 required-height">
        ...
        <b-col style="max-width: 80px"> </b-col>
      </b-row>
    </div>
  </b-container>
</template>
